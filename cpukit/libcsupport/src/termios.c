...
{
  return tty->handler.mode == TERMIOS_IRQ_SERVER_DRIVEN || tty->handler.mode == TERMIOS_TASK_DRIVEN;
}
    ...
    const rtems_termios_callbacks *callbacks)
{
  if (tty == NULL)
  {
    ...
    {
      tty->handler.first_open = callbacks->firstOpen != NULL ? rtems_termios_callback_firstOpen : NULL;
      tty->handler.last_close = callbacks->lastClose != NULL ? rtems_termios_callback_lastClose : NULL;
      tty->handler.poll_read = callbacks->pollRead != NULL ? rtems_termios_callback_pollRead : NULL;
      tty->handler.write = callbacks->write != NULL ? rtems_termios_callback_write : NULL;
      tty->handler.set_attributes = callbacks->setAttributes != NULL ? rtems_termios_callback_setAttributes : NULL;
      tty->flow.stop_remote_tx = callbacks->stopRemoteTx != NULL ? rtems_termios_callback_stopRemoteTx : NULL;
      tty->flow.start_remote_tx = callbacks->startRemoteTx != NULL ? rtems_termios_callback_startRemoteTx : NULL;
...
rtems_termios_ioctl(void *arg)
{
  ...
  case TIOCSETAF:
    ...
    {
      sc = (*tty->handler.set_attributes)(tty->device_context, &tty->termios) ? RTEMS_SUCCESSFUL : RTEMS_IO_ERROR;
    }
    break;
...
fillBufferQueue(struct rtems_termios_tty *tty)
{
  ...
  while (wait)
  {
           ...
           (tty->ccount < (CBUFSIZE - 1)))
    {
      ...
      if (((tty->rawInBuf.Tail - newHead) % tty->rawInBuf.Size) < tty->lowwater)
      {
        tty->flow_ctrl &= ~FL_IREQXOF;
        /* if tx stopped and XON should be sent... */
        if (((tty->flow_ctrl & (FL_MDXON | FL_ISNTXOF)) == (FL_MDXON | FL_ISNTXOF)) && ((tty->rawOutBufState == rob_idle) || (tty->flow_ctrl & FL_OSTOP)))
        {
...
rtems_termios_refill_transmitter(struct rtems_termios_tty *tty)
{
  ...
  if ((tty->flow_ctrl & (FL_MDXOF | FL_IREQXOF | FL_ISNTXOF)) == (FL_MDXOF | FL_IREQXOF))
  {
